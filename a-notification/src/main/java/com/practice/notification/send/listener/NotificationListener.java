...
/**
 * Kafka {@code notification} 토픽을 소비하는 리스너입니다. 원본 {@code NotificationTopicHandler}에 대응합니다.
 *
 * <p><b>사내 Kafka 라이브러리 → Spring Kafka 대체</b>: 원본은 사내 라이브러리 사내 Kafka 라이브러리의 {@code @TopicHandler}가
 * 이 역할을 합니다. 여기서는 표준 {@code @KafkaListener}로 같은 일을 합니다. 사내 어노테이션이 감추던
 * "토픽 구독 → 역직렬화 → 핸들러 호출"이 표준 API로 드러납니다.
 *
 * <p>메시지는 JSON String으로 받아 {@link NotificationEvent}로 역직렬화합니다(원본도 실제 String 직렬화).
