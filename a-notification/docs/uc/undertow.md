...
이 문서는 특정 UC가 아니라 **프로젝트 전역 기반**인 내장 웹서버 Undertow를 손으로 확인하는 노트입니다. 개념·아키텍처·"왜 Tomcat 대신 쓰는가"는 원본 분석 문서에 충실히 있으니 **여기서는 이 프로젝트에서 실제로 비교/실측하는 절차**만 둡니다.

- **개념 역링크**: Drive `비공개 분석 노트/원본 커스텀/concepts/undertow.md`
  (NIO 이벤트 루프, 서블릿 스펙 호환, Spring Boot 교체 메커니즘, 스레드 모델 비교, 왜 유리한가 — 모두 여기에 있음)
...
기본값 검증 (튜닝 안 했으니 기본값이 뜬다 — 근거는 개념 문서):
...
> 연구 결론(왜 이 워크로드에 Undertow가 유리한가 등)은 개념 문서 §"왜 Tomcat 대신 Undertow인가"에 있으니, **여기엔 실측해서 새로 안 것만** 덧붙인다.
