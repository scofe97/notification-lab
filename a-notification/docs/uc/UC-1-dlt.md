...
리스너에서 예외 → DefaultErrorHandler가 재시도(1초 간격 2회)
  → 소진되면 DeadLetterPublishingRecoverer가 원본 메시지를 {원본토픽}.DLT 로 발행
```
...
# DLT 내용을 처음부터 읽기 (원본 메시지가 그대로 들어있다)
...
```bash
cd ~/notification-lab && docker compose up -d kafka-ui   # 8100 포트 (지금은 기본 미기동)
...
- DLT 전용 `@KafkaListener(topics="notification.DLT")`를 하나 더 두고 → 로그만 남기거나 → 조건부로 원본 토픽에 재발행(재처리).
