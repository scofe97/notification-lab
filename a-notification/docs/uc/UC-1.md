...
원본 분석 문서 루트: Drive `비공개 분석 노트/`. 경로에 공백·한글이 있어 링크가 깨질 수 있으니 상대경로로 찾습니다.

| 기술 | 원본 분석 문서 | UC-1에서의 역할 |
|------|----------------|-----------------|
| Kafka 소비 | `원본 커스텀/concepts/kafka.md` | `notification` 토픽 소비 (사내 Kafka 라이브러리 `@TopicHandler` 대체) |
| Undertow | `원본 커스텀/concepts/undertow.md` | 내장 웹서버 (Tomcat 대체) |
| Caffeine | `사내 CMP 플랫폼/concepts/caffeine-cache.md` | 채널 설정 조회 캐시 |
| OpenFeign | `사내 CMP 플랫폼/concepts/open-feign.md` | 외부 발송 API 호출 |
| Resilience4j | `사내 CMP 플랫폼/concepts/resilience4j-circuit-breaker.md` | 발송 회로차단 |
| 사내 Kafka 라이브러리 구조 | 메모리 `feedback_사내 Kafka 라이브러리_jar_dev_branch` | 사내 Kafka 라이브러리 (대체 대상) |

### 사내 Kafka 라이브러리 → Spring Kafka 대체 매핑 (핵심 학습)

원본은 사내 Kafka 라이브러리를 씁니다. 그 추상화를 표준 Spring Kafka로 풀어 본 매핑입니다.
...
| `send/config/KafkaConsumerConfig` | 재시도 횟수(2회)·간격(1s)·DLT 토픽명(`{원본}.DLT`) | 재시도 정책이 곧 "몇 번 실패해야 포기하나"의 정의 |
...
```bash
cd ~/notification-lab && docker compose up -d          # kafka·wiremock 등
cd a-notification && ./gradlew bootRun             # 8092에 Undertow로 뜬다
...
> 손으로 해보며 "분석 문서로 읽었을 때와 무엇이 달랐는가"를 남깁니다. 아래는 빈 골격입니다.
