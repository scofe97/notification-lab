...
| DB latency | cache miss 상태에서 PostgreSQL 조회 지연 또는 낮은 HikariCP pool 설정 | DB query latency·pending 증가, 외부 API 정상, 처리 latency·lag 증가 | Kafka lag가 DB 병목에서도 발생함을 확인 |
...
| UC-07 | PostgreSQL latency·HikariCP | query latency·active/pending·lag | `07-db-latency.md` |
...
- `dashboards/`: pipeline overview, external API·CircuitBreaker, cache·PostgreSQL 대시보드
