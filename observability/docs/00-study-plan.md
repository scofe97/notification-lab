...
3단계의 목적은 Kafka 알림 파이프라인에서 지연·실패·DLT·CircuitBreaker·캐시·DB 문제가 발생했을 때, 어떤 증거로 원인을 좁히는지 학습하는 것입니다. 서비스 기능을 넓히는 대신 관측 신호와 운영 판단을 결과물로 남깁니다. 채널 설정과 발송 이력은 PostgreSQL에 저장하며, cache miss가 DB·HikariCP 신호로 이어지는 흐름을 관측 대상으로 삼습니다.
...
각 주차의 아키텍처 그림과 주요 관측 신호·키워드는 [관측 아키텍처 §주차별 발전 아키텍처](01-architecture.md)에 있습니다.
...
관측 스터디의 산출물은 모두 `observability/` 아래에 모입니다. 설계 문서는 `docs/`, 실험으로 얻은 결과물은 나머지 폴더에 쌓습니다.

```text
notification-lab/
  apps/
    notification-service/              # ✅ 관측 대상 (PostgreSQL 저장)
    notification-scenario-runner/      # 🟡 신호·장애 발생기 스캐폴딩
  infra/
    compose.yaml                       # 컴포넌트 compose 집계
    compose/                           # Kafka·PostgreSQL·OpenSearch·WireMock 등 분리 파일
  observability/
    docs/                              # 관측 설계 문서 (이 문서 포함)
...
- **반드시**: consumer lag, retry/DLT, WireMock 5xx·timeout·delay, CircuitBreaker 상태, Caffeine hit/miss, PostgreSQL query latency·HikariCP, consumer rebalance, traceId 기반 log-trace 연결, Grafana 대시보드, 장애 실험 문서, Runbook
- **후반 (JVM 축)**: GC 압박, 스레드 블로킹, OOM·heap dump — 관측 UC-09~11로 정규 편입(2026-07-21). 서비스 신호(외부 API·DB·consumer)로 설명되지 않는 병목을 다루므로 기본 관측이 자리 잡은 뒤 진행합니다.
