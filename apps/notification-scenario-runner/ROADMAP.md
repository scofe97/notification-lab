...
전체 프로젝트 단계는 [루트 ROADMAP](../../ROADMAP.md), 관측 시나리오의 증거와 판단 절차는 [시나리오·운영 절차](../../observability/docs/02-scenarios-and-operations.md)에 있습니다.
...
| 1. Kafka 메시지 발행 | ✅ 완료 (2026-07-23) | 시나리오 타입에 따라 알림 이벤트를 생성하고 발행합니다. | `ScenarioType`(normal·burst·poison 3변형), `EventPublishPort`+Kafka 어댑터(동기 발행), CLI 원샷 트리거. E2E 스모크로 소비·DLT 확인 |
| 2. WireMock 장애 제어 | ✅ 완료 (2026-07-23) | 외부 의존성의 응답 조건을 실험 중 바꿉니다. | `FaultInjectionPort`+admin API 어댑터 — 5xx·delay 주입(priority 1), `fault-reset`으로 정상 복원 |
...
| 5. 관측 UC 연동 | 🔜 | 스터디 시나리오를 반복 실행하고 증거를 남깁니다. | UC-01~UC-12 실행 절차(서비스 신호 축 UC-01~08 + JVM 축 UC-09~11 + Loki 축 UC-12), `observability/experiments/` 기록, Dashboard·Runbook 교차 링크 |
...
5. Stage 5에서 [관측 UC](../../observability/docs/02-scenarios-and-operations.md)의 재현법·증거·원인 판단을 실제 기록으로 채웁니다.

## 비목표

- 운영 환경의 부하 테스트 도구를 대체하지 않습니다.
- 알림 발송 로직, retry 정책, DLT 재처리는 `notification-service`의 책임입니다.
- Docker Compose·LGMT 구성 자체의 구현은 [관측 스터디 계획](../../observability/docs/00-study-plan.md)에서 관리합니다.
