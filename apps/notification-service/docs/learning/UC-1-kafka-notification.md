...
> 상태: 이해 확인 완료 (2026-07-20) — Phase 1~5 통과, 실측 실습과 CircuitBreaker 수정·재검증 포함. 남은 후속: 부분 실패 중복 발송 실측, 캐시 무효화 영향 (후속 검증 표 참조).
...
질의응답 전에 "무엇이 만들어졌는지"만 봅니다. 각 판단이 **왜** 그렇게 됐는지는 아래 Phase 질문에서 다루므로 여기서는 설명하지 않습니다.

```mermaid
flowchart LR
    UP["상류 시스템"] -->|"JSON 이벤트"| K["Kafka notification 토픽"]
    K --> L["NotificationListener"]
    L --> S["NotificationSendService<br/>채널별 그룹화·수신거부 필터"]
    S --> CS["ChannelSettingService<br/>Caffeine 캐시 경유 H2 조회"]
    S --> F["NotificationSendClient<br/>POST /send/채널"]
    F --> W["WireMock 발송 API"]
    L -.->|"처리 예외"| EH["DefaultErrorHandler<br/>1초 간격 2회 재시도"]
    EH -.->|"소진"| DLT["notification.DLT"]
```

| 구성요소 | 파일 (`send/` 아래) | 하는 일 |
|----------|--------------------|---------|
| 진입점 | `listener/NotificationListener.java` | `notification` 토픽 구독, JSON을 `NotificationEvent`로 역직렬화. HTTP 진입점은 없음 |
| 발송 서비스 | `service/NotificationSendService.java` | 수신자를 `ChannelType`(SMS·ALIMTALK·EMAIL)별로 그룹화, 수신 거부자 제외, 채널 단위로 발송 |
| 채널 설정 | `channel/ChannelSettingService.java` 외 2 | `(userId, channelType)` 수신 여부를 H2에서 조회, `@Cacheable`. 설정이 없으면 수신으로 간주 |
| 캐시 스펙 | `config/CacheConfig.java` | Caffeine — TTL 10분, 최대 10,000개, `recordStats()` |
| 외부 호출 | `remote/NotificationSendClient.java` | OpenFeign `POST /send/{channel}` → WireMock(8110) |
| 실패 처리 | `config/KafkaConsumerConfig.java` | 리스너가 던진 예외를 1초 간격 2회 재시도, 소진 시 `notification.DLT` 적재 |
| 회로차단 | `remote/NotificationSendCaller.java` + `application.yml` | `@CircuitBreaker(name = "notificationSend")`, 최근 5건 중 50% 실패 기준. 프록시 경유를 위해 서비스와 분리된 빈 |
| 데이터 계약 | `domain/` record 3종 | `NotificationEvent`(수신자 목록 포함)·`ChannelType`·`SendResult` |
...
| 코드 | [NotificationListener](../../src/main/java/com/practice/notification/send/listener/NotificationListener.java), [NotificationSendService](../../src/main/java/com/practice/notification/send/service/NotificationSendService.java), [KafkaConsumerConfig](../../src/main/java/com/practice/notification/send/config/KafkaConsumerConfig.java) | 실제 호출 순서와 예외 전파 |
...
[`NotificationListener.onMessage`](../../src/main/java/com/practice/notification/send/listener/NotificationListener.java)는 Kafka에서 `String` 메시지를 받고 `ObjectMapper`로 `NotificationEvent`를 만듭니다. 이 단계는 단순 변환이 아니라, 이후 서비스가 `eventId`, 제목, 내용, 수신자 목록이라는 도메인 데이터를 다루게 만드는 경계입니다.

메시지가 JSON이 아니면 이 메서드에서 예외가 납니다. 여기서는 예외를 잡지 않습니다. 실패를 성공처럼 끝내지 않고 리스너 컨테이너의 실패 처리로 넘기기 위해서입니다.

### 2. 수신자를 채널별 발송 단위로 묶습니다

[`NotificationSendService.send`](../../src/main/java/com/practice/notification/send/service/NotificationSendService.java)는 수신자를 `ChannelType`별로 그룹화합니다. 한 채널의 목적지를 한 `SendRequest`로 묶어 `/send/sms`, `/send/email`처럼 채널별 외부 API를 호출할 수 있게 합니다.

이 구조에서 발송 횟수는 수신자 수가 아니라 활성화된 채널 수에 가깝습니다. 같은 채널의 수신자가 여러 명이면 목적지 목록 하나로 발송합니다.

### 3. 채널 설정은 먼저 캐시에서 찾습니다

[`ChannelSettingService.isEnabled`](../../src/main/java/com/practice/notification/send/channel/ChannelSettingService.java)는 `(userId, channelType)`을 키로 `@Cacheable`을 사용합니다. 처음 조회하는 조합은 저장소를 읽고, 같은 조합의 다음 조회는 Caffeine 캐시를 사용합니다. 설정이 없으면 현재 구현은 `true`, 즉 수신 허용으로 간주합니다.
...
[`NotificationSendCaller.callSend`](../../src/main/java/com/practice/notification/send/remote/NotificationSendCaller.java)는 채널 이름으로 경로를 정하고 Feign 클라이언트를 호출합니다. 성공하면 `SendResult`를 돌려주고, 리스너는 채널별 결과를 한 번의 완료 로그로 남깁니다. 이 메서드가 서비스가 아닌 별도 빈에 있는 이유는 아래 Phase 3에서 다룹니다.
...
[`KafkaConsumerConfig`](../../src/main/java/com/practice/notification/send/config/KafkaConsumerConfig.java)는 `DefaultErrorHandler`에 1초 간격 2회 재시도를 설정합니다. 재시도가 모두 실패하면 `DeadLetterPublishingRecoverer`가 원본 파티션과 같은 번호의 `notification.DLT`로 레코드를 보냅니다. DLT를 다시 소비하거나 재처리하는 기능은 현재 범위에 없습니다.
...
읽는 순서에 주의합니다. 이 진단은 코드를 읽어 세운 가설이고, 실측(24건 실패에도 차단 0회)이 이를 확정했으며, 수정 뒤 재실험(5건에서 OPEN)이 원인 귀속까지 증명했습니다. 가설·관측·확정을 뭉뚱그리지 않는 것이 이 문서의 규칙입니다.
...
| 부분 실패 시 중복 발송 | 미실측 | 2채널 중 1채널만 5xx일 때 성공 채널의 수신 횟수 확인, `eventId` 기반 멱등 처리 설계 검토 |
| 캐시 무효화 부재의 영향 | 미실측 | 설정 변경 후 최대 10분간 옛 값 발송 — UC-4 CRUD에서 `@CacheEvict`와 함께 다룸 |

## Phase 4 · 실측 실습 기록 (2026-07-20)
