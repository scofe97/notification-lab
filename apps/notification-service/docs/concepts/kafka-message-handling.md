...
"컨테이너가 커밋을 대행한다"는 말은 AOP처럼 프록시가 끼어든다는 뜻이 아닙니다. AOP는 다른 코드가 내 빈을 호출할 때 프록시가 그 사이에 서는 구조인데, 리스너는 방향이 반대입니다 — **호출하는 쪽이 애초에 컨테이너입니다.** `@KafkaListener`를 붙이면 Spring이 `KafkaMessageListenerContainer`라는 폴링 루프 스레드를 띄우고, 리스너 메서드는 그 루프 안에서 호출당하는 콜백이 됩니다.

```text
while (true) {
    records = consumer.poll()          // 브로커에서 레코드 획득
    for (record : records) {
        onMessage(record)              // 리스너 메서드는 이 루프의 콜백
        consumer.commit(offset + 1)    // 정상 리턴하면 컨테이너가 여기서 커밋 (RECORD 모드)
    }
}
```

커밋 코드는 콜백 호출 *다음 줄*에 컨테이너가 이미 갖고 있으므로, 프록시로 무엇을 끼워 넣을 필요가 없습니다. 예외가 나면 다음 줄에 도달하지 못할 뿐이고, 이것이 "정상 리턴 = 커밋"의 정체입니다.
